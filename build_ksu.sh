...
# (دالة التثبيت fetch_tools و install_deps تظل كما هي لضمان استقرار السكربت)
install_deps() {
    echo "--- Installing Dependencies ---"
    sudo apt update && sudo apt install -y git curl zip wget make gcc g++ bc
    # تضاف بقية الحزم هنا كما في السكربت الأول
}
...
fetch_tools() {
    if [[ ! -d "$TC_DIR/clang-r530567" ]]; then
        ...
        wget "https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+archive/refs/heads/main/clang-r530567.tar.gz" -O "$TC_DIR/clang.tar.gz"
        tar xf "$TC_DIR/clang.tar.gz" -C "$TC_DIR/clang-r530567" && rm "$TC_DIR/clang.tar.gz"
    fi
    rm -rf "$TC_DIR/AnyKernel3" && git clone "$AK3_REPO" "$TC_DIR/AnyKernel3"
}
...
build_kernel() {
    ...
    # إضافة كافة الـ Exports لنسخة KSU
    export ARCH=arm64 && export LLVM=1 && export BRANCH="android11" && export KMI_GENERATION=2
    export DEPMOD=depmod && export STOP_SHIP_TRACEPRINTK=1 && export IN_KERNEL_MODULES=1
    export DO_NOT_STRIP_MODULES=1 && export TRIM_NONLISTED_KMI=0 && export KMI_SYMBOL_LIST_ADD_ONLY=1
    export KMI_SYMBOL_LIST_STRICT_MODE=0 && export KMI_ENFORCED=0
    ...
    export LOCALVERSION="-KSU-$BRANCH-$KMI_GENERATION-$COMREV-rio-$VARIANT"
    
    echo "--- Building KSU for $DEVICE ---"
    START=$(date +%s)
...
gen_anykernel() {
    ...
    cd "$AK3_DIR" && zip -r9 "RIO_KSU_$(date +%Y%m%d)_${VARIANT}.zip" ./* -x .git* README.md
    mkdir -p "$TC_DIR/RIO/FINAL" && mv *.zip "$TC_DIR/RIO/FINAL/" && cd "$SRC_DIR"
}

install_deps && fetch_tools
