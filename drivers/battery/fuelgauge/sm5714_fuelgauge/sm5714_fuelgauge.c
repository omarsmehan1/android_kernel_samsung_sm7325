...
/*		break; */
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_BATTERY_ID:
			val->intval = fuelgauge->battery_data->bat_id;
			break;
		default:
			return -EINVAL;
		}
...
*/
	...
	fuelgauge->pdata->bat_id_gpio_01 = of_get_named_gpio(np, "fuelgauge,bat_id_gpio_01", 0);
	if (fuelgauge->pdata->bat_id_gpio_01 > 0) {
		int bat_id, bat_id_01, bat_id_02;

		bat_id_01 = gpio_get_value(fuelgauge->pdata->bat_id_gpio_01);
		pr_info("%s : bat_id_1(%d)\n", __func__, bat_id_01);
		fuelgauge->pdata->bat_id_gpio_02 = of_get_named_gpio(np, "fuelgauge,bat_id_gpio_02", 0);
		if (fuelgauge->pdata->bat_id_gpio_02 > 0) {
			bat_id_02 = gpio_get_value(fuelgauge->pdata->bat_id_gpio_02);	
			pr_info("%s : bat_id_2(%d)\n", __func__, bat_id_02);
			if (bat_id_01)
				bat_id = (bat_id_02 ? BATTERY_ID_01 : BATTERY_ID_03);
			else
				bat_id = (bat_id_02 ? BATTERY_ID_02 : BATTERY_ID_04);
		} else {
			bat_id = (bat_id_01 ? BATTERY_ID_01 : BATTERY_ID_02);
		}
		fuelgauge->battery_data->bat_id = bat_id;
	} else {
		fuelgauge->battery_data->bat_id = 0;
	}
	pr_info("%s: bat_id = %d\n", __func__, fuelgauge->battery_data->bat_id);
