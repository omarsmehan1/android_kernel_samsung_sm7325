...
build_kernel() {
    ...
    # إضافة كافة الـ Exports لنسخة KSU-NEXT
    ...
    export LOCALVERSION="-KSUNEXT-$BRANCH-$KMI_GENERATION-$COMREV-rio-$VARIANT"
    
    echo "--- Building KSU-NEXT for $DEVICE ---"
    START=$(date +%s)
...
gen_anykernel() {
    AK3_DIR="$TC_DIR/RIO/work_ksunext"
    ...
    cd "$AK3_DIR" && zip -r9 "RIO_KSUNEXT_$(date +%Y%m%d)_${VARIANT}.zip" ./* -x .git* README.md
